...
def main():
    ...
    for i in range(len(Y_test)):
        if Y_pred_test[i] != Y_test.iloc[i]:
          print(f"Student Index: {X_test.index[i]} Predicted output: {Y_pred_test[i]} \n Expected output: {Y_test.iloc[i]}")   
        else:
            correct += 1
    ...
    misclassified = X_test[Y_pred_test != Y_test.to_numpy()]

    print("Misclassified Students:")
    print(misclassified)

    misclassified_count = sum(Y_pred_test != Y_test.to_numpy())
    print("Number of misclassified students:", misclassified_count)
